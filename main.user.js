...
(function () {
  ...
  class YoutubeReelRemover {
    ...
    #removeReelNavigator() {
      const navigator = document.querySelector(REEL_NAV_SELECTOR);
      if (navigator != null) {
        navigator.remove();
      }
    }
    ...
    run() {
      this.#removeExtraVideos();
      this.#removeReelNavigator();
    }
  }

  // ======================
  // Website Ready Detector
  // ======================
  function waitWebsiteReady(callback) {
    const contentLoaded = () => document.querySelector(REEL_VIDEO_SELECTOR) != null;
