// ==UserScript==
// @name        Hide Youtube Short Scrolling
// @namespace   Violentmonkey Scripts
// @match       https://www.youtube.com/*
// @icon        data:image/png;base64,
// @grant       none
// @version     1.0
// @author      -
// @description 4/17/2026, 10:10:39 AM
// ==/UserScript==

(function () {
  "use strict";

  const config = {
    REEL_NAV_SELECTOR: ".navigation-container",
    REEL_VIDEO_SELECTOR: ".reel-video-in-sequence-new",
  };

  // ====================
  // Youtube Reel Remover
  // ====================
  class YoutubeReelRemover {
    /**
     * Removes next videos in the reel scroll menu
     * @param {HTMLElement} root Container for reel videos
     */
    #removeExtraVideos(root) {
      let reels;

      reels = root.querySelectorAll(config.REEL_VIDEO_SELECTOR);
      if (reels.length < 2) {
        return;
      }

      reels.forEach((reel, index) => {
        if (index > 0) {
          reel.remove();
        }
      });
    }

    /**
     * Removes the navigation buttons for reel scrolling
     */
    #removeReelNavigator() {
      const navigator = document.querySelector(config.REEL_NAV_SELECTOR);
      if (navigator != null) {
        navigator.remove();
      }
    }

    /**
     * Wait for the reels root to be available and return it.
     *
     * **Timeout after 10 seconds**
     * @returns {Promise<HTMLElement>}
     */
    async #awaitReelsRoot(timeout = 10000) {
      return new Promise((resolve, reject) => {
        let interval, timeoutId;

        timeoutId = setTimeout(() => {
          clearInterval(interval);
          reject();
        }, timeout);

        interval = setInterval(() => {
          const root = document.querySelector(config.REEL_VIDEO_SELECTOR)?.parentNode;
          if (root != null) {
            clearTimeout(timeoutId);
            clearInterval(interval);
            resolve(root);
          }
        }, 100);
      });
    }

    run() {
      const currentUrl = window.location.pathname;

      if (!currentUrl.startsWith("/shorts/")) {
        console.debug("[YoutubeReelRemover] Not a shorts page, skipping");
        return;
      }

      this.#awaitReelsRoot()
        .then((root) => {
          console.debug("[YoutubeReelRemover] Reels root found; running removeExtraVideos and removeReelNavigator");
          this.#removeExtraVideos(root);
          this.#removeReelNavigator();
        })
        .catch(() => {
          console.error("[YoutubeReelRemover] Timeout: Reels root not found");
        });
    }
  }

  function main() {
    const remover = new YoutubeReelRemover();

    navigation.addEventListener("navigate", () => {
      console.debug("[YoutubeReelRemover] navigate event");
      requestAnimationFrame(() => {
        remover.run();
      });
    });

    remover.run();
  }

  main();
})();

// After the last refactor, YoutubeReelRemover no longer runs when the reels root updates its descedent list.
// if Youtube eventually replaces those elements, create a fix by adding a MutationObserver to the root and
// removing the observer onNavigate.
